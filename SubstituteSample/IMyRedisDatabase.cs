...
{
    public interface IMyRedisDatabase : IDatabase
    {
    }

    public class MyRedisDatabase : IDatabase
    {
        private dynamic connect;

        public MyRedisDatabase(dynamic connect)
        {
        ...
        public async Task<long> SortedSetLengthAsync(RedisKey key, double min = -1, double max = 1, Exclude exclude = Exclude.None,
            CommandFlags flags = CommandFlags.None)
        {
            throw new NotImplementedException();
        }
        ...
        public async Task<RedisValue[]> SortedSetRangeByScoreAsync(RedisKey key, double start = -1, double stop = 1, Exclude exclude = Exclude.None,
            Order order = Order.Ascending, long skip = 0, long take = -1, CommandFlags flags = CommandFlags.None)
        {
            throw new NotImplementedException();
        }

        public async Task<SortedSetEntry[]> SortedSetRangeByScoreWithScoresAsync(RedisKey key, double start = -1, double stop = 1,
        ...
        public async Task<bool> StringSetAsync(RedisKey key, RedisValue value, TimeSpan? expiry = null, When when = When.Always,
            CommandFlags flags = CommandFlags.None)
        {
            throw new NotImplementedException();
        }
        ...
        {
            return connect.KeyExists(key, flags);
        ...
        public RedisValue[] SortedSetRangeByScore(RedisKey key, double start = -1, double stop = 1,
            Exclude exclude = Exclude.None, Order order = Order.Ascending, long skip = 0, long take = -1, CommandFlags flags = CommandFlags.None)
        {
            throw new NotImplementedException();
        }
        ...
        {
            Console.WriteLine("写入redis了，但是你拿不出！哈哈！");
            var data = (IDatabase)connect.GetDatabase();
            return data.StringSet(key, value, expiry, when, flags);
        }
